...
public class suLyDonHangAdmin_Adapter extends RecyclerView.Adapter<suLyDonHangAdmin_Adapter.ViewHolder> {
    ArrayList<model_hoaDon> arrListHoaDon;
    ArrayList<model_Account> arrListAccount;
    xac_Nhan_Don_hang_Activity context;
    boolean checkLoai = false;

    public suLyDonHangAdmin_Adapter(ArrayList<model_hoaDon> arrListHoaDon, ArrayList<model_Account> arrListAccount, xac_Nhan_Don_hang_Activity context, boolean checkLoai) {
        this.arrListHoaDon = arrListHoaDon;
        this.arrListAccount = arrListAccount;
    ...
    public ViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        ...
        return new suLyDonHangAdmin_Adapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull suLyDonHangAdmin_Adapter.ViewHolder holder, int position) {
        model_hoaDon arrHoaDon = arrListHoaDon.get(position);
        model_Account arrAccount = null;
        for (int i = 0; i < arrListAccount.size(); i++) {
            if (arrHoaDon.getIdKhachHang().equals(arrListAccount.get(i).getId())) {
                arrAccount = arrListAccount.get(i);
